...
for ( j in sampleNames){
  ...
  highest_identity <- order(dat.best$identity, decreasing = T)
  
  dat.select <- dat.best[highest_identity, ]
  row.names(dat.select[1,]) <- j
  
  
  
  best.genes.protein.Zm00001d049443_mRNA1[[j]] <-dat.select[1, ]
}
...
for ( j in sampleNames){
  ...
  best.genes.protein.Zm00001d049443_mRNA2[[j]] <-dat.select[1, ]
}
